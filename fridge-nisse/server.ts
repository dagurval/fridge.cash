...
async function mainLoop(address: string): Promise<void> { let lastPriceUpdate = 0;
    ...
    while (true) {
        ...
        const balance_fiat = balance_satoshi / price as number;
        log(chalk.green(`Balance on address is ${balance_satoshi} sats`));
        log(chalk.green(`Fiat balance is ${balance_fiat} ${CONVERSION_CURRENCY}`));
        sleep.msleep(5000);
    }
}
